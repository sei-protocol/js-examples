...
import { SIGN_ARBITRARY_TEXT, useWalletTests } from './hooks/useWalletTests';
...
const WalletVerification = ({}: WalletInfoProps) => {
	...
	const renderSignArbitrary = () => {
		return (
			<CodeExecute title='Sign Arbitrary'
									 text={`await connectedWallet.signArbitrary('${chainId}', '${accounts[0].address}', '${SIGN_ARBITRARY_TEXT}');`}
									 response={signArbitraryResponse} error={signArbitraryError} onClickExecute={testSignArbitrary} />
		);
	};

	const renderGetAccounts = () => {
		return (
			<CodeExecute title='GET ACCOUNTS'
									 text={`const offlineSigner = await connectedWallet.getOfflineSigner('${chainId}');\nofflineSigner.getAccounts()`}
									 response={getAccountsResponse} error={getAccountsError} onClickExecute={testGetAccounts} />
		);
	};

	const renderGetOfflineSigner = () => {
		return (
			<CodeExecute title='GET OFFLINE SIGNER' text={`await connectedWallet.getOfflineSigner('${chainId}');`}
									 response={getOfflineSignerResponse} error={getOfflineSignerError}
	...
	const renderGetOfflineSignerAuto = () => {
		return (
			<CodeExecute title='GET OFFLINE SIGNER AUTO' text={`await connectedWallet.getOfflineSignerAuto('${chainId}');`}
									 response={getOfflineSignerAutoResponse} error={getOfflineSignerAutoError}
